...
# Build variables
BINARY_NAME=maestro-mobile-runner
VERSION?=$(shell git describe --tags --always --dirty 2>/dev/null || echo "dev")
COMMIT?=$(shell git rev-parse --short HEAD 2>/dev/null || echo "none")
BUILD_DATE?=$(shell date -u +"%Y-%m-%dT%H:%M:%SZ")
LDFLAGS=-ldflags "-X github.com/luoshixin93-sudo/maestro-mobile-runner/pkg/cli.Version=${VERSION} -X github.com/luoshixin93-sudo/maestro-mobile-runner/pkg/cli.Commit=${COMMIT} -X github.com/luoshixin93-sudo/maestro-mobile-runner/pkg/cli.BuildDate=${BUILD_DATE}"

# Install directory (MAESTRO_MOBILE_RUNNER_HOME layout: bin/, cache/, drivers/)
INSTALL_DIR=$(HOME)/.maestro-mobile-runner
